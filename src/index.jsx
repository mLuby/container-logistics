...
function city (node) { return node.city_name }
...
axios.all([edgesPromise, nodesPromise]).then(compose(createGraph, addEdgesToMap, fst)).then(() => {
  const node = nodesObj[34]
  const time = 15
  const nodes = nodesInRange(node, time)
